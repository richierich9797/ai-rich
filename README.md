# ai-rich｜AI 跨境电商百日百万 ——刘瑞奇

> 我是刘瑞奇。6年跨境电商亿级卖家，这是我的 AI 跨境电商开源项目：从零开始，公开记录百日百万挑战，逐步分享可复用的方法与工具。

**从抖音来的朋友，欢迎来到刘瑞奇的项目主页。** 认准 GitHub 账号 [@richierich9797](https://github.com/richierich9797) 与本仓库 [ai-rich](https://github.com/richierich9797/ai-rich)。

**当前状态：项目筹备，仅搭建仓库框架，尚未发布业务内容或可安装的 Skills。**

## 项目介绍

围绕「100 天挑战 100 万」的实践，记录 AI 在跨境电商中的应用，并将实践中形成的方法整理为 Skills、工作流、提示词、模板与知识库，供大家学习和使用。

「100 天挑战 100 万」是项目挑战目标，不代表已取得的成果。具体统计口径与挑战规则将在后续说明中公布。

## 内容目录

| 目录 | 预留用途 | 当前状态 |
| --- | --- | --- |
| [skills/](skills/) | 可由 AI Agent 调用的技能 | 空目录 |
| [workflows/](workflows/) | 跨境电商任务流程与操作步骤 | 空目录 |
| [prompts/](prompts/) | 可复用的提示词 | 空目录 |
| [templates/](templates/) | 表格、文档和任务模板 | 空目录 |
| [knowledge/](knowledge/) | 整理后的方法与知识资料 | 空目录 |
| [journal/](journal/) | 挑战过程、实验记录与复盘 | 空目录 |
| [docs/](docs/) | 项目说明与使用指南 | 空目录 |

各目录中的 `.gitkeep` 仅用于让 Git 保留空目录。

## 使用与参与

项目尚无安装步骤。后续发布可用内容时，再补充对应说明。

可以点击页面右上角的 **Star** 收藏本项目，方便下次找到。

欢迎通过 [Issues](https://github.com/richierich9797/ai-rich/issues) 提出需求与建议，或通过 Pull Request 参与共建。参与前请阅读 [贡献说明](CONTRIBUTING.md)。

## 发起人

**刘瑞奇（刘瑞奇Richy）**

[抖音](https://www.douyin.com/user/MS4wLjABAAAAsns-O5aJqaE3yUbzXxBLcyFKDq-W2L2VZz-qRaDYNLDet0SxKG2PPAMO4V0gpof0) · [X / Twitter](https://x.com/richy_liu0202) · [YouTube](https://www.youtube.com/channel/UCWQfWQ8bGOBTpn3g7MHv1qQ) · [GitHub](https://github.com/richierich9797)

抖音号：`62609410812`

## 许可证

本项目采用 [MIT License](LICENSE)。允许使用、修改、分发和商业使用，包括用于经营店铺；再分发时须保留版权声明与许可证。具体条款以 `LICENSE` 为准。

Copyright © 2026 刘瑞奇。

